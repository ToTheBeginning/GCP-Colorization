...
parser.add_argument('--output', type=str, default='assets/predicted_label_for_user_image.txt')
...
def main():
    ...
    with open(os.path.join(args.output_dir, args.output), 'w') as out_file:
        filenames = loader.dataset.filenames(basename=True)
        for filename, label in zip(filenames, topk_ids):
            out_file.write('{0} placeholder {1}\n'.format(os.path.splitext(filename)[0], label[0]))
